...
Vector2D Vector2D::operator+(const Vector2D& rhs) const { return Vector2D(vec.x + rhs.vec.x, vec.y + rhs.vec.y); }
Vector2D& Vector2D::operator+=(const Vector2D& rhs) { vec.x += rhs.vec.x; vec.y += rhs.vec.y; return *this; }
